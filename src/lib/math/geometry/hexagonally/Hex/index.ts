...
export class Hex<TValue> {
    ...
    add(b: Hex<TValue>): Hex<TValue> {
        ...
        return new Hex({ q: q + b.coordinates.q, r: r + b.coordinates.r, s: s + b.coordinates.s }, this.value);
    }
    ...
    private static fromAxial(coordinates: Axial): Cuboid {
        return { q: coordinates.q, r: coordinates.r, s: -coordinates.q - coordinates.r, };
    }

    private static fromCartesian(coordinates: Cartesian): Cuboid {
        const { row, col, offset } = coordinates;
        return Hex.fromAxial({ q: col, r: row - (col + offset * (col & 1)) / 2, });
    }
